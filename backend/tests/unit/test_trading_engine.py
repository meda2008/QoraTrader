...
@patch('src.database.get_db')
@patch('src.events.bus.publish_event')
def test_trading_engine_start_stop(mock_publish_event, mock_get_db):
    """
...
@patch('src.database.get_db')
@patch('src.events.bus.publish_event')
def test_order_submission(mock_publish_event, mock_get_db):
    """
    ...
    # Create a test order
    from src.models.base import OrderStatus
    order = Order(
    ...
    order_id = asyncio.run(engine.submit_order(order))
...
@patch('src.database.get_db')
@patch('src.events.bus.publish_event')
def test_order_cancellation(mock_publish_event, mock_get_db):
    """
    ...
    success = asyncio.run(engine.cancel_order(order_id))
...
def test_position_management(mock_get_db):
    ...
    position = asyncio.run(engine.get_position(account_id, symbol))
    assert position is None
    
    # Get all positions for account (should return empty list initially)
    positions = asyncio.run(engine.get_all_positions(account_id))
    assert positions == []
...
@patch('src.database.get_db')
@patch('src.events.bus.publish_event')
def test_trading_engine_state_after_error(mock_publish_event, mock_get_db):
    """
...
@patch('src.database.get_db')
@patch('src.events.bus.publish_event')
def test_trading_engine_concurrent_operations(mock_publish_event, mock_get_db):
    """
    ...
    order_ids = [asyncio.run(engine.submit_order(order)) for order in orders]
...
@patch('src.database.get_db')
@patch('src.events.bus.publish_event')
def test_trading_engine_cleanup(mock_publish_event, mock_get_db):
    """
